...
class SendingServer extends Model
{
    ...
    {
        return $this->hasMany('MailChamp\Model\TrackingLog', 'sending_server_id')->orderBy('created_at', 'asc');
    }
    ...
    {
        return $this->belongsTo('MailChamp\Model\BounceHandler');
    }
    ...
    {
        $class_name = '\MailChamp\Model\\'.self::$serverMapping[$server->type];
    ...
    public function displayQuota()
    {
        ...
        return $this->quota_value.' / '.$this->quota_base.' '.trans('messages.'.\MailChamp\Library\Tool::getPluralPrase($this->quota_unit, $this->quota_base));
    }
    ...
    public function sendTestEmail($params)
    {
        ...
        $message->setId($msgId);
        $message->getHeaders()->addTextHeader('X-MailChamp-Message-Id', $msgId); // this header is required for SendGrid API sending server
