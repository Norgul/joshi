...
class Subscriber extends Model
{
    ...
    {
        return $this->hasMany('MailChamp\Model\EmailVerification');
    }

    public function emailVerification()
    {
        return $this->hasOne('MailChamp\Model\EmailVerification');
    }

    public function campaign()
    {
        return $this->belongsTo('MailChamp\Model\Campaign');
    }

    public function subscriberFields()
    {
        return $this->hasMany('MailChamp\Model\SubscriberField');
    }
    ...
    public static function import($user, $list)
    {
        ...
        foreach ($lines as $key => $line) {
            ...
            // authorize
            if ($user->cannot('create', new \MailChamp\Model\Subscriber(['mail_list_id' => $list->id]))) {
                ...
                if ($valid) {
                    ...
                    foreach ($parts as $key => $value) {
                        ...
                        if (isset($headers[$key])) {
                            ...
                            if (is_object($lf)) {
                                //// save fields
                                $lfv = new \MailChamp\Model\SubscriberField(array(
                                                'field_id' => $lf->id,
    ...
    public function copy($list, $type = 'keep')
    {
        ...
        if (!is_object($copy)) {
            ...
            $copy = \MailChamp\Model\Subscriber::find($copy->id);
        } else {
            // return if keep
            if ($type == 'keep') {
                return false;
    ...
    {
        $query = \MailChamp\Model\TrackingLog::where('tracking_logs.subscriber_id', '=', $this->id);
    ...
    {
        $query = \MailChamp\Model\OpenLog::leftJoin('tracking_logs', 'tracking_logs.message_id', '=', 'open_logs.message_id')
    ...
    {
        $query = \MailChamp\Model\ClickLog::leftJoin('tracking_logs', 'tracking_logs.message_id', '=', 'click_logs.message_id')
            ->where('tracking_logs.subscriber_id', '=', $this->id);
