...
class Admin extends Model
{
    ...
    {
        return $this->belongsTo('MailChamp\Model\AdminGroup');
    }

    public function customers()
    {
        return $this->hasMany('MailChamp\Model\Customer');
    }

    public function templates()
    {
        return $this->hasMany('MailChamp\Model\Template');
    }
    ...
    {
        return $this->belongsTo('MailChamp\Model\User', 'creator_id');
    }
    ...
    public function getColorScheme()
    {
        if (!empty($this->color_scheme)) {
            ...
            return \MailChamp\Model\Setting::get('backend_scheme');
        }
    }
    ...
    {
        $query = \MailChamp\Model\Customer::getAll();

        if (!$this->user->can('readAll', new \MailChamp\Model\Customer())) {
    ...
    {
        $query = \MailChamp\Model\Subscription::select('subscriptions.*')
    ...
    {
        $query = \MailChamp\Model\Log::select('logs.*')->join('customers', 'customers.id', '=', 'logs.customer_id')
    ...
    public function createCustomerAccount($admin)
    {
        if (!$this->hasCustomerAccount()) {
            // Create customer
            $customer = new \MailChamp\Model\Customer();
            ...
            // Add plan
            $plan = \MailChamp\Model\Plan::getDefaultPlan();
            $subscription = new \MailChamp\Model\Subscription();
            $subscription->status = \MailChamp\Model\Subscription::STATUS_ACTIVE;
            ...
            // Start at default
            $subscription->start_at = \MailChamp\Library\Tool::dateTime(\Carbon\Carbon::now());
    ...
    {
        return \MailChamp\Model\Plan::getAllActive($this);
    }
    ...
    {
        $query = \MailChamp\Model\PaymentMethod::getAll()
            ->where('payment_methods.status', '=', \MailChamp\Model\PaymentMethod::STATUS_ACTIVE);

        if (!$this->can('readAll', new \MailChamp\Model\PaymentMethod())) {
    ...
    {
        $query = \MailChamp\Model\Admin::getAll()
            ->where('admins.status', '=', \MailChamp\Model\Admin::STATUS_ACTIVE);

        if (!$this->can('readAll', new \MailChamp\Model\Admin())) {
    ...
    {
        $query = \MailChamp\Model\AdminGroup::getAll();

        if (!$this->can('readAll', new \MailChamp\Model\AdminGroup())) {
    ...
    {
        $query = \MailChamp\Model\SendingServer::getAll();

        if (!$this->can('readAll', new \MailChamp\Model\SendingServer())) {
    ...
    {
        $query = \MailChamp\Model\SendingDomain::getAll();

        if (!$this->can('readAll', new \MailChamp\Model\SendingDomain())) {
    ...
    {
        $query = \MailChamp\Model\Campaign::getAll();

        if (!$this->can('readAll', new \MailChamp\Model\Customer())) {
    ...
    {
        $query = \MailChamp\Model\Automation::getAll();
    ...
    {
        $query = \MailChamp\Model\Subscriber::getAll();
    ...
    public function addEmaillToBlacklist($email)
    {
        ...
        if (\MailChamp\Library\Tool::isValidEmail($email)) {
            $exist = \MailChamp\Model\Blacklist::whereNull('customer_id')->where('email', '=', $email)->count();
            if (!$exist) {
    ...
    public function getSubaccountSendingServers()
    {
        ...
        $query = $query->whereIn('type', \MailChamp\Model\SendingServer::getSubAccountTypes());
