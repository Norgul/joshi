...
class Page extends Model
{
    ...
    {
        return $this->belongsTo('MailChamp\Model\Layout');
    }
    ...
    public function renderContent($values = null, $subscriber = null)
    {
        ...
        $this->content = str_replace('{SUBSCRIBE_URL}', \MailChamp\Model\Setting::get('url_root') . action('PageController@signUpForm', $this->mailList->uid, false), $this->content);

        // SUBSCRIBE CONFIRM URL
        if (isset($subscriber)) {
            // [SUBSCRIBE_CONFIRM_URL]
            $this->content = str_replace('{SUBSCRIBE_CONFIRM_URL}', \MailChamp\Model\Setting::get('url_root') . action('PageController@signUpConfirmationThankyou', array('list_uid' => $this->mailList->uid, 'uid' => $subscriber->uid, 'code' => $subscriber->getSecurityToken('subscribe-confirm')), false), $this->content);
            ...
            // UBSUBSCRIBE URL
            $this->content = str_replace('{UNSUBSCRIBE_URL}', \MailChamp\Model\Setting::get('url_root') . action('PageController@unsubscribeForm', array('list_uid' => $this->mailList->uid, 'uid' => $subscriber->uid, 'code' => $subscriber->getSecurityToken('unsubscribe')), false), $this->content);

            $this->content = str_replace('{UPDATE_PROFILE_URL}', \MailChamp\Model\Setting::get('url_root') . action('PageController@profileUpdateForm', array('list_uid' => $this->mailList->uid, 'uid' => $subscriber->uid, 'code' => $subscriber->getSecurityToken('update-profile')), false), $this->content);
    ...
    public function getTransformedSubject($subscriber)
    {
        ...
        $subject = str_replace('{SUBSCRIBE_URL}', \MailChamp\Model\Setting::get('url_root') . action('PageController@signUpForm', $this->mailList->uid, false), $subject);

        // SUBSCRIBE CONFIRM URL
        if (isset($subscriber)) {
            // [SUBSCRIBE_CONFIRM_URL]
            $subject = str_replace('{SUBSCRIBE_CONFIRM_URL}', \MailChamp\Model\Setting::get('url_root') . action('PageController@signUpConfirmationThankyou', array('list_uid' => $this->mailList->uid, 'uid' => $subscriber->uid, 'code' => $subscriber->getSecurityToken('subscribe-confirm')), false), $subject);
            ...
            // UBSUBSCRIBE URL
            $subject = str_replace('{UNSUBSCRIBE_URL}', \MailChamp\Model\Setting::get('url_root') . action('PageController@unsubscribeForm', array('list_uid' => $this->mailList->uid, 'uid' => $subscriber->uid, 'code' => $subscriber->getSecurityToken('unsubscribe')), false), $subject);

            $subject = str_replace('{UPDATE_PROFILE_URL}', \MailChamp\Model\Setting::get('url_root') . action('PageController@profileUpdateForm', array('list_uid' => $this->mailList->uid, 'uid' => $subscriber->uid, 'code' => $subscriber->getSecurityToken('update-profile')), false), $subject);
