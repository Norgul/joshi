...
use MailChamp\Notifications\ResetPassword;
...
class User extends Authenticatable
{
    ...
    {
        return $this->hasOne('MailChamp\Model\Customer');
    }

    public function admin()
    {
        return $this->hasOne('MailChamp\Model\Admin');
    }

    public function systemJobs()
    {
        return $this->hasMany('MailChamp\Model\SystemJob')->orderBy('created_at', 'desc');
    }
    ...
    {
        $layout = \MailChamp\Model\Layout::where('alias', 'registration_confirmation_email')->first();
        $token = $this->getToken();
        $redirect_url = urlencode($redirect_url);

        $layout->content = str_replace('{ACTIVATION_URL}', \MailChamp\Model\Setting::get('url_root') . action('UserController@activate', ['token' => $token, 'redirect' => $redirect_url], false), $layout->content);
        $layout->content = str_replace('{CUSTOMER_NAME}', $name, $layout->content);
        
        \Mail::to(json_decode(json_encode(['email' => $this->email, 'name' => trans('messages.to_email_name')])))->send(new \MailChamp\Mail\RegistrationConfirmationMailer($layout->content, $layout->subject));
    }
    ...
    {
        return $this->hasOne('MailChamp\Model\userActivation');
    }
    ...
    {
        $token = \MailChamp\Model\UserActivation::getToken();

        $userActivation = $this->userActivation;

        if (!is_object($userActivation)) {
            $userActivation = new \MailChamp\Model\UserActivation();
