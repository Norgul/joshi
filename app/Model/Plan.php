...
class Plan extends Model
{
    ...
    {
        return $this->hasMany('MailChamp\Model\PlansSendingServer');
    }

    public function currency()
    {
        return $this->belongsTo('MailChamp\Model\Currency');
    }

    public function subscriptions()
    {
        return $this->hasMany('MailChamp\Model\Subscription');
    }

    public function plansEmailVerificationServers()
    {
        return $this->hasMany('MailChamp\Model\PlansEmailVerificationServer');
    }
    ...
    public static function select2($request)
    {
        ...
        $query = \MailChamp\Model\Plan::getAllActive()->orderBy('custom_order', 'asc');
        if (isset($request->q)) {
            $keyword = $request->q;
            $query = $query->where(function ($q) use ($keyword) {
                $q->orwhere('plans.name', 'like', '%'.$keyword.'%');
        ...
        // Read all check
        if ($request->user()->admin && !$request->user()->admin->can('readAll', new \MailChamp\Model\Plan())) {
            $query = $query->where('plans.admin_id', '=', $request->user()->admin->id);
        }

        foreach ($query->limit(20)->get() as $plan) {
            $data['items'][] = ['id' => $plan->uid, 'text' => htmlspecialchars($plan->name).'|||'.htmlspecialchars(\MailChamp\Library\Tool::format_price($plan->price, $plan->currency->format))];
        }
    ...
    public static function getAllActiveWithDefault($admin = null)
    {
        ...
        if (isset($admin) && !$admin->can('readAll', new \MailChamp\Model\Plan())) {
    ...
    public function displayFrequencyTime()
    {
        ...
        return $this->frequency_amount.' '.\MailChamp\Library\Tool::getPluralPrase($this->frequency_unit, $this->frequency_amount);
    }
    ...
    public function fillPlansEmailVerificationServers($params)
    {
        ...
        foreach ($params as $key => $param) {
            if ($param['check']) {
                ...
                $row = new \MailChamp\Model\PlansEmailVerificationServer();
                $row->plan_id = $this->id;
    ...
    {
        $planMethodlist = \MailChamp\Model\PaymentMethod::getAllActive();
