...
class Subscription extends Model
{
    ...
    public function rules()
    {
        ...
        if ($this->getOption('sending_server_option') == \MailChamp\Model\Plan::SENDING_SERVER_OPTION_SUBACCOUNT) {
    ...
    {
        return $this->belongsTo('MailChamp\Model\Plan');
    }
    ...
    {
        return $this->belongsTo('MailChamp\Model\PaymentMethod');
    }
    public function subscriptionsSendingServers()
    {
        return $this->hasMany('MailChamp\Model\SubscriptionsSendingServer');
    }
    public function payments()
    {
        return $this->hasMany('MailChamp\Model\Payment');
    }
    public function subscriptionsEmailVerificationServers()
    {
        return $this->hasMany('MailChamp\Model\SubscriptionsEmailVerificationServer');
    }
    ...
    public function subAccount()
    {
        ...
        return $this->belongsTo('MailChamp\Model\SubAccountSendGrid');
    }
    ...
    {
        return $this->getOption('sending_server_option') == \MailChamp\Model\Plan::SENDING_SERVER_OPTION_SUBACCOUNT;
    }
    ...
    public static function filter($request)
    {
        ...
        if (!empty($filters)) {
            ...
            if (!empty($filters['plan_uid'])) {
                $plan = \MailChamp\Model\Plan::findByUid($filters['plan_uid']);
    ...
    {
        $this->plan_id = $this->plan_id ? $this->plan_id : (!empty($params['plan_uid']) ? \MailChamp\Model\Plan::findByUid($params['plan_uid'])->id : null);
        $this->customer_id = $this->customer_id ? $this->customer_id : (isset($params['customer_uid']) ? \MailChamp\Model\Customer::findByUid($params['customer_uid'])->id : null);
        $this->payment_method_id = $this->payment_method_id ? $this->payment_method_id : (!empty($params['payment_method_uid']) ? \MailChamp\Model\PaymentMethod::findByUid($params['payment_method_uid'])->id : null);
        ...
        if (isset($this->plan_id)) {
            ...
            switch ($plan->frequency_unit) {
                ...
                case 'year':
                    ...
                    break;
                case 'unlimited':
                    $this->start_at = \MailChamp\Library\Tool::dateTime(\Carbon\Carbon::now());
                    $this->end_at = null;
                    break;
            }
        }
    ...
    public static function defaultOptions()
    {
        $options = [
            ...
            'sending_server_option' => \MailChamp\Model\Plan::SENDING_SERVER_OPTION_SYSTEM,
            'sending_server_subaccount_uid' => null,
            'api_access' => 'yes',
        ];

        // Sending server types
        foreach (\MailChamp\Model\SendingServer::types() as $key => $type) {
    ...
    public static function defaultUnlimitedOptions()
    {
        $options = [
            ...
            'all_sending_server_types' => 'yes',
            'sending_server_types' => [],
            'sending_server_option' => \MailChamp\Model\Plan::SENDING_SERVER_OPTION_OWN,
            'sending_server_subaccount_uid' => null,
            'api_access' => 'yes',
        ];
    ...
    public function displayQuota()
    {
        if ($this->getOption('sending_quota') == -1) {
            ...
            return strtolower(\MailChamp\Library\Tool::format_number($this->getOption('sending_quota')).' '.trans('messages.'.\MailChamp\Library\Tool::getPluralPrase('email', $this->getOption('sending_quota'))).' / '.$this->getOption('sending_quota_time').' '.trans('messages.'.\MailChamp\Library\Tool::getPluralPrase($this->getOption('sending_quota_time_unit'), $this->getOption('sending_quota'))));
        }
    }
    ...
    public function displayTotalQuota()
    {
        if ($this->getOption('email_max') == -1) {
            ...
            return \MailChamp\Library\Tool::format_number($this->getOption('email_max'));
        }
    }
    ...
    public function displayMaxList()
    {
        if ($this->getOption('list_max') == -1) {
            ...
            return \MailChamp\Library\Tool::format_number($this->getOption('list_max'));
        }
    }
    ...
    public function displayMaxSubscriber()
    {
        if ($this->getOption('subscriber_max') == -1) {
            ...
            return \MailChamp\Library\Tool::format_number($this->getOption('subscriber_max'));
        }
    }
    ...
    public function displayMaxCampaign()
    {
        if ($this->getOption('campaign_max') == -1) {
            ...
            return \MailChamp\Library\Tool::format_number($this->getOption('campaign_max'));
        }
    }
    ...
    public function displayMaxSizeUploadTotal()
    {
        if ($this->getOption('max_size_upload_total') == -1) {
            ...
            return \MailChamp\Library\Tool::format_number($this->getOption('max_size_upload_total'));
        }
    }
    ...
    public function displayFileSizeUpload()
    {
        if ($this->getOption('max_file_size_upload') == -1) {
            ...
            return \MailChamp\Library\Tool::format_number($this->getOption('max_file_size_upload'));
        }
    }
    ...
    {
        if ($this->getOption('sending_server_option') != \MailChamp\Model\Plan::SENDING_SERVER_OPTION_OWN) {
    ...
    public function updateEmailVerificationServers($servers)
    {
        ...
        foreach ($servers as $key => $param) {
            if ($param['check']) {
                $server = \MailChamp\Model\EmailVerificationServer::findByUid($key);
                $row = new \MailChamp\Model\SubscriptionsEmailVerificationServer();
                $row->subscription_id = $this->id;
    ...
    public function setPaid($params = [])
    {
        ...
        // description
        if (!empty($params)) {
            $payment = new \MailChamp\Model\Payment();
            ...
            $payment->status = \MailChamp\Model\Payment::STATUS_SUCCESS;
            $payment->action = \MailChamp\Model\Payment::ACTION_PAID;
    ...
    public function setUnPaid($description = '')
    {
        ...
        if (!empty($description)) {
            ...
            $payment->payment_method_id = null;
            $payment->status = \MailChamp\Model\Payment::STATUS_SUCCESS;
            $payment->action = \MailChamp\Model\Payment::ACTION_UNPAID;
            $payment->payment_method_name = '';
            $payment->order_id = $this->getOrderID();
            $payment->save();
        }
    ...
    {
        $strs = [$this->plan_name];
        $strs[] = '('.\MailChamp\Library\Tool::formatDate($this->start_at);
        $strs[] = ' - '.\MailChamp\Library\Tool::formatDate($this->end_at).')';
        if ($this->isCurrent()) {
    ...
    {
        if (
            in_array($payment_method->type, [\MailChamp\Model\PaymentMethod::TYPE_BRAINTREE_PAYPAL, \MailChamp\Model\PaymentMethod::TYPE_BRAINTREE_CREDIT_CARD])
    ...
    public function createSubAccount()
    {
        ...
        try {
            ...
            $server = \MailChamp\Model\SendingServer::findByUid($this->getOption('sending_server_subaccount_uid'));
            $account = \MailChamp\Model\SubAccountSendGrid::setup([
