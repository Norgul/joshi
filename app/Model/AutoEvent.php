...
class AutoEvent extends Model
{
    ...
    {
        $relation = $this->hasMany('MailChamp\Model\AutoTrigger');
    ...
    {
        return $this->belongsToMany('MailChamp\Model\AutomatedCampaign', 'auto_campaigns', 'auto_event_id', 'campaign_id');
    }

    public function originCampaigns()
    {
        return $this->belongsToMany('MailChamp\Model\Campaign', 'auto_campaigns');
    }
    ...
    {
        return $this->hasMany('MailChamp\Model\AutoTrigger');
    }
        ...
        switch ($this->event_type) {
            case self::TYPE_SPECIFIC_DATETIME:
                ...
                if (!empty($params['specific_day']) && !empty($params['specific_time'])) {
                    $send_at = \MailChamp\Library\Tool::systemTimeFromString($params['specific_day'].' '.$params['specific_time'], \Auth::user()->timezone);
                    $this->updateData([
            ...
            case self::TYPE_WEEKLY_RECURRING:
                ...
                if (!empty($params['weekly_recurring_time'])) {
                    $send_at = \MailChamp\Library\Tool::systemTimeFromString('2000-01-01 '.$params['weekly_recurring_time'], \Auth::user()->timezone);
                    $this->updateData([
            ...
            case self::TYPE_MONTHLY_RECURRING:
                ...
                if (!empty($params['monthly_recurring_time'])) {
                    $send_at = \MailChamp\Library\Tool::systemTimeFromString('2000-01-01 '.$params['monthly_recurring_time'], \Auth::user()->timezone);
                    $this->updateData([
                        'time' => $send_at->toAtomString(),
                    ]);
                }
        ...
        switch ($this->event_type) {
            case self::TYPE_SPECIFIC_DATETIME:
                $msg = trans('messages.on_the_specific_date', [
                    'date' => \MailChamp\Library\Tool::dayStringFromTimestamp(\MailChamp\Library\Tool::dateTimeFromString($this->getDataValue('datetime'))),
                    'time' => \MailChamp\Library\Tool::timeStringFromTimestamp(\MailChamp\Library\Tool::dateTimeFromString($this->getDataValue('datetime'))),
                ]);
                break;
            case self::TYPE_WEEKLY_RECURRING:
                $msg = trans('messages.weekly_recurring_description', [
                    'weekdays' => implode(', ', \MailChamp\Library\Tool::numberArrayToWeekdaysArray($this->getDataValue('weekdays'))),
                    'weeks' => implode(', ', \MailChamp\Library\Tool::numberArrayToWeeksArray($this->getDataValue('weeks'))),
                    'months' => implode(', ', \MailChamp\Library\Tool::numberArrayToMonthsArray($this->getDataValue('months'))),
                    'time' => \MailChamp\Library\Tool::timeStringFromTimestamp(\MailChamp\Library\Tool::dateTimeFromString($this->getDataValue('time'))),
                ]);
                break;
            case self::TYPE_MONTHLY_RECURRING:
                $msg = trans('messages.monthly_recurring_description', [
                    'days' => implode(', ', \MailChamp\Library\Tool::getDayNamesFromArrayOfNumber($this->getDataValue('days'))),
                    'months' => implode(', ', \MailChamp\Library\Tool::numberArrayToMonthsArray($this->getDataValue('months'))),
                    'time' => \MailChamp\Library\Tool::timeStringFromTimestamp(\MailChamp\Library\Tool::dateTimeFromString($this->getDataValue('time'))),
                ]);
                break;
    ...
    {
        return $this->hasOne('MailChamp\Model\AutoEvent', 'previous_event_id');
    }
    ...
    {
        return $this->belongsTo('MailChamp\Model\AutoEvent', 'previous_event_id');
    }
