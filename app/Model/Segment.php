...
class Segment extends Model
{
    ...
    {
        return $this->hasMany('MailChamp\Model\SegmentCondition');
    }
    ...
    {
        $query = \MailChamp\Model\Subscriber::select('subscribers.*', 'email_verifications.result AS verify_result');
    ...
    public function updateCacheDelayed()
    {
        $existed = SystemJobModel::getNewJobs()
                       ->where('name', \MailChamp\Jobs\UpdateSegmentJob::class)
                       ->where('data', $this->id)
                       ->exists();
        ...
        if (!$existed) {
            dispatch(new \MailChamp\Jobs\UpdateSegmentJob($this));
        }
    }
