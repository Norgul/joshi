...
use MailChamp\Library\RouletteWheel;
use MailChamp\Library\StringHelper;
use MailChamp\Exceptions\VerificationProcessCancelledException;
...
class MailList extends Model
{
    ...
    {
        return $this->hasMany('MailChamp\Model\Field');
    }
    ...
    {
        return $this->hasMany('MailChamp\Model\Segment');
    }

    public function pages()
    {
        return $this->hasMany('MailChamp\Model\Page');
    }
    ...
    {
        return $this->hasMany('MailChamp\Model\Subscriber');
    }

    public function campaigns()
    {
        return $this->belongsToMany('MailChamp\Model\Campaign', 'campaigns_lists_segments', 'mail_list_id', 'campaign_id');
    }
    ...
    {
        return $this->belongsToMany('MailChamp\Model\Automation', 'automations_lists_segments', 'mail_list_id', 'automation_id');
    }
    ...
    public function createDefaultFieds()
    {
        ...
        $this->fields()->create([
                            ...
                            'tag' => \MailChamp\Model\Field::formatTag(trans('messages.first_name_tag')),
                            'required' => false,
                            'visible' => true,
                        ]);
        ...
        $this->fields()->create([
                            ...
                            'tag' => \MailChamp\Model\Field::formatTag(trans('messages.last_name_tag')),
                            'required' => false,
                            'visible' => true,
                        ]);
    }
    ...
    public function longName($cache = false)
    {
        ...
        return $this->name.' - '.$count.' '.trans('messages.'.\MailChamp\Library\Tool::getPluralPrase('subscriber', $count)).'';
    }
    ...
    {
        return \MailChamp\Model\SystemJob::where('name', '=', "MailChamp\Jobs\ImportSubscribersJob")
    ...
    {
        return \MailChamp\Model\SystemJob::where('name', '=', "MailChamp\Jobs\ExportSubscribersJob")
            ->where('data', 'like', '%"mail_list_uid":"'.$this->uid.'"%');
    }
    ...
    {
        return \MailChamp\Model\SystemJob::where('name', '=', "MailChamp\Jobs\ExportSegmentsJob")
    ...
    public static function export($list, $customer, $job)
    {
        ...
        for ($page = 0; $page <= ceil($total / $num); ++$page) { // ceil($total/$num)
            ...
            foreach ($list->subscribers()->skip($page * $num)->take($num)->get() as $key => $item) {
                ...
                $data[] = \MailChamp\Library\Tool::arrayToCsv($cols, ',');
    ...
    public static function exportSegments($list, $customer, $job)
    {
        ...
        $headers = $json->fields;
        $segment = \MailChamp\Model\Segment::findByUid($json->uid);
    ...
    {        
        ...
        $layout = \MailChamp\Model\Layout::where('alias', 'sign_up_confirmation_email')->first();
        $send_page = \MailChamp\Model\Page::findPage($list, $layout);
    ...
    public function send($message, $params = [])
    {
        ...
        $message->getHeaders()->addTextHeader('X-MailChamp-Message-Id', StringHelper::generateMessageId(StringHelper::getDomainFromEmail($this->from_email)));
    ...
    public function sendSubscriptionWelcomeEmail($subscriber)
    {
        ...
        $layout = \MailChamp\Model\Layout::where('alias', 'sign_up_welcome_email')->first();
        $send_page = \MailChamp\Model\Page::findPage($list, $layout);
        $this->sendMail($subscriber, $send_page, $send_page->getTransformedSubject($subscriber));
    }
    ...
    public function sendUnsubscriptionNotificationEmail($subscriber)
    {
        ...
        $layout = \MailChamp\Model\Layout::where('alias', 'unsubscribe_goodbye_email')->first();
        $send_page = \MailChamp\Model\Page::findPage($list, $layout);
        $this->sendMail($subscriber, $send_page, $send_page->getTransformedSubject($subscriber));
    }
    ...
    public function sendProfileUpdateEmail($subscriber)
    {
        ...
        $layout = \MailChamp\Model\Layout::where('alias', 'profile_update_email')->first();
        $send_page = \MailChamp\Model\Page::findPage($list, $layout);
        $this->sendMail($subscriber, $send_page, $send_page->getTransformedSubject($subscriber));
    }
    ...
    private function readCsv($file)
    {
        try {
            ...
            $encoding = \MailChamp\Library\StringHelper::detectEncoding($file);

            if ($encoding == false) {
                MailLog::warning("Cannot detect file's encoding: {$file}");
            } elseif ($encoding != 'UTF-8') {
                MailLog::warning("Convert from {$encoding} to UTF-8");
                \MailChamp\Library\StringHelper::toUTF8($file, $encoding);
            } else {
    ...
    {
        return $this->hasMany('MailChamp\Model\MailListsSendingServer');
    }
    ...
    public function getSendingServers()
    {
        ...
        if ($this->customer->getOption('sending_server_option') == \MailChamp\Model\Plan::SENDING_SERVER_OPTION_OWN) {
        ...
        } elseif ($this->customer->getOption('sending_server_option') == \MailChamp\Model\Plan::SENDING_SERVER_OPTION_SYSTEM) {
            // Check if has sending servers for current subscription
            if (is_object($subscription)) {
                if ($subscription->getOption('all_sending_servers') == 'yes') {
                    if (\MailChamp\Model\SendingServer::getAllAdminActive()->count()) {
                        $result = \MailChamp\Model\SendingServer::getAllAdminActive()->get()->map(function ($server) {
                            return [$server->id, '100'];
                        });
                    }
    ...
    public function queueForVerification($serverId)
    {
        ...
        if (is_null($job)) {
            $job = (new \MailChamp\Jobs\VerifyMailListJob($this->id, $serverId));
            dispatch($job);
        } else {
    ...
    {
        $job = SystemJob::where('name', 'MailChamp\Jobs\VerifyMailListJob')
