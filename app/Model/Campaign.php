...
use DB;
use MailChamp\Exceptions\CampaignPausedException;
use MailChamp\Exceptions\CampaignErrorException;
...
class Campaign extends AbsCampaign
{
    ...
    {
        $this->ready();
        $job = (new \MailChamp\Jobs\RunCampaignJob($this))->delay($this->getDelayInSeconds());
        dispatch($job);
    }
    ...
    public function clearAllJobs()
    {
        ...
        $systemJobs = SystemJob::where('name', 'MailChamp\Jobs\RunCampaignJob')->where('data', $this->id)->get();
