...
class CustomerGroup extends Model
{
    ...
    {
        return $this->hasMany('MailChamp\Model\CustomerGroupSendingServer');
    }

    public function sending_servers()
    {
        return $this->belongsToMany('MailChamp\Model\SendingServer', 'customer_group_sending_servers');
    }
    ...
    public function displayQuota()
    {
        if ($this->getOption('sending_quota') == -1) {
            ...
            return $this->getOption('sending_quota').' '.trans('messages.'.\MailChamp\Library\Tool::getPluralPrase('email', $this->getOption('sending_quota'))).' / '.$this->getOption('sending_quota_time').' '.trans('messages.'.\MailChamp\Library\Tool::getPluralPrase($this->getOption('sending_quota_time_unit'), $this->getOption('sending_quota')));
        }
    }
