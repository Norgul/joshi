...
abstract class AbsCampaign extends Model
{
    ...
    {
        if ($this->isAuto()) {
            return $this->autoEvent()->automation->belongsTo('MailChamp\Model\MailList', 'default_mail_list_id');
        } else {
            return $this->belongsTo('MailChamp\Model\MailList', 'default_mail_list_id');
        }
    }
    ...
    {
        return $this->belongsToMany('MailChamp\Model\MailList', 'campaigns_lists_segments');
    }
    ...
    {
        return $this->belongsTo('MailChamp\Model\SendingDomain', 'tracking_domain_id');
    }
    ...
    {
        return $this->belongsToMany('MailChamp\Model\AutoEvent', 'auto_campaigns');
    }
    ...
    {
        return $this->belongsToMany('MailChamp\Model\Link', 'campaign_links');
    }
    ...
    {
        return $this->hasMany('MailChamp\Model\TrackingLog');
    }
    ...
    {
        return $this->hasMany('MailChamp\Model\CampaignsListsSegment');
    }
    ...
    public function prepareEmail($subscriber, $server)
    {
        ...
        $msgId = $customHeaders['X-MailChamp-Message-Id'];
    ...
    public function getCustomHeaders($subscriber, $server)
    {
        ...
        return array(
            'X-MailChamp-Campaign-Id' => $this->uid,
            'X-MailChamp-Subscriber-Id' => $subscriber->uid,
            'X-MailChamp-Customer-Id' => $this->customer->uid,
            'X-MailChamp-Message-Id' => $msgId,
            'X-MailChamp-Sending-Server-Id' => $server->uid,
    ...
    {
        return \MailChamp\Web\Subscriber::selectRaw(DB::getTablePrefix().'list_subscriber.*, COUNT('.DB::getTablePrefix().'campaign_track_unsubscribe.id) AS openCount')
    ...
    public function openUniqHours($start = null, $end = null)
    {
        ...
        return $query->orderBy('open_logs.created_at', 'asc')->get()->groupBy(function ($date) {
            return \MailChamp\Library\Tool::dateTime($date->created_at)->format('H'); // grouping by hours
        });
    }
    ...
    public function fillAttributes($params)
    {
        ...
        if (isset($params['tracking_domain_uid'])) {
            $tracking_domain = \MailChamp\Model\SendingDomain::findByUid($params['tracking_domain_uid']);
