...
use MailChamp\Library\QuotaTrackerFile;
...
class Customer extends Model
{
    ...
    {
        return $this->belongsTo('MailChamp\Model\Contact');
    }
    ...
    {
        return $this->belongsTo('MailChamp\Model\Admin');
    }

    public function lists()
    {
        return $this->hasMany('MailChamp\Model\MailList')->orderBy('created_at', 'desc');
    }

    public function language()
    {
        return $this->belongsTo('MailChamp\Model\Language');
    }

    public function campaigns()
    {
        return $this->hasMany('MailChamp\Model\Campaign')->orderBy('created_at', 'desc');
    }

    public function sentCampaigns()
    {
        return $this->hasMany('MailChamp\Model\Campaign')->where('status', '=', 'done')->orderBy('created_at', 'desc');
    }

    public function subscribers()
    {
        return $this->hasManyThrough('MailChamp\Model\Subscriber', 'MailChamp\Model\MailList');
    }

    public function logs()
    {
        return $this->hasMany('MailChamp\Model\Log')->orderBy('created_at', 'desc');
    }

    public function trackingLogs()
    {
        return $this->hasMany('MailChamp\Model\TrackingLog')->orderBy('created_at', 'asc');
    }

    public function automations()
    {
        return $this->hasMany('MailChamp\Model\Automation');
    }

    public function subscriptions()
    {
        return $this->hasMany('MailChamp\Model\Subscription')->orderBy('created_at', 'desc');
    }

    public function sendingDomains()
    {
        return $this->hasMany('MailChamp\Model\SendingDomain');
    }
    ...
    {
        return $this->hasMany('MailChamp\Model\EmailVerificationServer');
    }
    ...
    {
        return $this->hasMany('MailChamp\Model\Blacklist');
    }
    ...
    {
        $query = \MailChamp\Model\Subscriber::leftJoin('mail_lists', 'mail_lists.id', '=', 'subscribers.mail_list_id')
    ...
    public function getOptions()
    {
        ...
        if (is_object($subscription)) {
            ...
            return \MailChamp\Model\Subscription::defaultOptions();
        }
    }
    ...
    public function getColorScheme()
    {
        if (!empty($this->color_scheme)) {
            ...
            return \MailChamp\Model\Setting::get('frontend_scheme');
        }
    }
    ...
    public static function select2($request)
    {
        ...
        $query = \MailChamp\Model\Customer::getAll()->leftJoin('users', 'users.id', '=', 'customers.user_id');
        if (isset($request->q)) {
            $keyword = $request->q;
            $query = $query->where(function ($q) use ($keyword) {
                $q->orwhere('customers.first_name', 'like', '%'.$keyword.'%')
        ...
        // Read all check
        if (!$request->user()->admin->can('readAll', new \MailChamp\Model\Customer())) {
    ...
    public function updateInformation($request)
    {
        ...
        $user = is_object($this->user) ? $this->user : new \MailChamp\Model\User();
    ...
    public function getCurrentSubscription()
    {
        if (!$this->currentSubscription) {
            ...
            ->where('subscriptions.status', '=', \MailChamp\Model\Subscription::STATUS_ACTIVE)
            ->first();
        }
    ...
    {
        return $subsciption = $this->subscriptions()
            ->where('status', '=', \MailChamp\Model\Subscription::STATUS_INACTIVE)
            ->first();
    }
    ...
    public function getNextScriptionStartAt()
    {
        ...
        if (is_object($last_subscription) && isset($last_subscription->end_at)) {
            ...
            return \MailChamp\Library\Tool::customerDateTime(\Carbon\Carbon::now());
        }
    }

    public function sendingServers()
    {
        return $this->hasMany('MailChamp\Model\SendingServer');
    }

    public function subAccounts()
    {
        return $this->hasMany('MailChamp\Model\SubAccount');
    }
    ...
    {
        $query = \MailChamp\Model\Customer::select('customers.*');

        if (isset($admin) && !$admin->can('readAll', new \MailChamp\Model\Customer())) {
    ...
    {
        return $this->sendingServers()->where('status', '=', \MailChamp\Model\SendingServer::STATUS_ACTIVE);
    }
    ...
    public function getNotOutdatedInactiveSubscriptions()
    {
        return $subsciption = $this->getNotOutdatedSubscriptions()
            ->where('subscriptions.status', '=', \MailChamp\Model\Subscription::STATUS_INACTIVE);
    }
    ...
    {
        return \MailChamp\Library\Tool::getDirectorySize(base_path('public/source/'.$this->user->uid)) / 1048576;
    }
    ...
    public function getContact()
    {
        if (is_object($this->contact)) {
            ...
            $contact = new \MailChamp\Model\Contact([
        ...
        // If customer dont have permission creating sending servers
            ...
            if (is_object($subscription)) {
                if ($subscription->getOption('all_email_verification_servers') == 'yes') {
                    $result = \MailChamp\Model\EmailVerificationServer::getAllAdminActive()->get()->map(function ($server) {
                        return $server;
                    });
                } else {
    ...
    public function getSendingDomains()
    {
        ...
        // If customer dont have permission creating sending domain
            ...
            // Check if has email verification servers for current subscription
            $result = \MailChamp\Model\SendingDomain::getAllAdminActive()->get()->map(function ($server) {
                return $server;
            });
        }
    ...
    {
        $query = \MailChamp\Model\MailList::getAll();
    ...
    {
        $allTypes = \MailChamp\Model\SendingServer::types();
        $types = [];
    ...
    {
        return \MailChamp\Model\SystemJob::where('name', '=', "MailChamp\Jobs\ImportBlacklistJob")
    ...
    public function getActiveImportBlacklistJobs()
    {
        return $this->getImportBlacklistJobs()
            ->where('status', '!=', \MailChamp\Model\SystemJob::STATUS_DONE)
            ->where('status', '!=', \MailChamp\Model\SystemJob::STATUS_FAILED)
            ->where('status', '!=', \MailChamp\Model\SystemJob::STATUS_CANCELLED);
    }
    ...
    public function addEmaillToBlacklist($email)
    {
        ...
        if (\MailChamp\Library\Tool::isValidEmail($email)) {
            $exist = $this->blacklists()->where('email', '=', $email)->count();
            if (!$exist) {
                $blacklist = new \MailChamp\Model\Blacklist();
    ...
    {
        return \MailChamp\Model\SendingDomain::getAllActive();
    }
}
