...
class Tool
{
    ...
    public static function dayStringFromTimestamp($timestamp)
    {
        if (isset($timestamp) && $timestamp != '0000-00-00 00:00:00') {
            ...
            $result = \MailChamp\Library\Tool::dateTime($timestamp)->format('Y-m-d');
        } else {
            $result = \MailChamp\Library\Tool::dateTime(\Carbon\Carbon::now())->format('Y-m-d');
        }
    ...
    public static function timeStringFromTimestamp($timestamp)
    {
        if (isset($timestamp) && $timestamp != '0000-00-00 00:00:00') {
            ...
            $result = \MailChamp\Library\Tool::dateTime($timestamp)->format('H:i');
        } else {
            $result = \MailChamp\Library\Tool::dateTime(\Carbon\Carbon::now())->format('H:i');
        }
    ...
    public static function cronjobUpdateController($request, $controller)
    {
        ...
        $setting_php_bin_path = \MailChamp\Model\Setting::get('php_bin_path');
        ...
        if (empty(\MailChamp\Model\Setting::get('remote_job_token'))) {
            \MailChamp\Model\Setting::set('remote_job_token', str_random(60));
        }
        ...
        if ($request->isMethod('post')) {
            ...
            if ($request->queue_driver == 'database') {
                ...
                // Check valid path
                $check = \MailChamp\Library\Tool::checkPHPBinPath($php_bin_path_value);
                if ($check != 'ok') {
                ...
                \MailChamp\Model\Setting::set('php_bin_path', $php_bin_path_value);
            ...
            if (in_array($queue_driver, ['database', 'async']) && config('queue.default') != $queue_driver) {
                \MailChamp\Model\Setting::setEnv('QUEUE_DRIVER', $queue_driver);
            }
