...
use MailChamp\Library\UpgradeManager;
...
class SettingController extends Controller
{
    ...
    public function general(Request $request)
    {
        ...
        // \MailChamp\Model\Setting::updateAll();
        $settings = \MailChamp\Model\Setting::getAll();
        if (null !== $request->old()) {
                ...
                if ($name != '_token' && isset($settings[$name])) {
                    ...
                    if ($name == 'site_logo_small' || $name == 'site_logo_big' || $name == 'site_favicon') {
                        if ($request->hasFile($name) && $request->file($name)->isValid()) {
                            \MailChamp\Model\Setting::uploadFile($request->file($name), $name);
                        }
                    } else {
                        if ($settings[$name]['cat'] == 'general' && $request->user()->admin->getPermission('setting_general') == 'yes') {
                            \MailChamp\Model\Setting::set($name, $value);
                        }
                    }
                }
            }
    ...
    public function cronjob(Request $request)
    {
        ...
        if($request->re_generate_remote_job_url) {
            ...
            \MailChamp\Model\Setting::set('remote_job_token', $remote_job_token);
            echo action('Controller@remoteJob', ['remote_job_token' => $remote_job_token]);
            return;
        }

        $respone = \MailChamp\Library\Tool::cronjobUpdateController($request, $this);
    ...
    public function mailer(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            foreach($env as $key => $value) {
                ...
                \MailChamp\Model\Setting::setEnv($key, $value);
            }
    ...
    public function updateUrls(Request $request)
    {
        ...
        \MailChamp\Model\Setting::set('url_root', url('/'));
        \MailChamp\Model\Setting::set('url_unsubscribe', action('CampaignController@unsubscribe', ['message_id' => 'MESSAGE_ID']));
        \MailChamp\Model\Setting::set('url_open_track', action('CampaignController@open', ['message_id' => 'MESSAGE_ID']));
        \MailChamp\Model\Setting::set('url_click_track', action('CampaignController@click', ['message_id' => 'MESSAGE_ID', 'url' => 'URL']));
        \MailChamp\Model\Setting::set('url_delivery_handler', action('DeliveryController@notify', ['stype' => '']));
        \MailChamp\Model\Setting::set('url_update_profile', action('PageController@profileUpdateForm', array(
            'list_uid' => 'LIST_UID',
            'uid' => 'SUBSCRIBER_UID',
            'code' => 'SECURE_CODE'))
        );
        \MailChamp\Model\Setting::set('url_web_view', action('CampaignController@webView', ['message_id' => 'MESSAGE_ID']));
    ...
    public function license(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            try {
                // Update license type
                \MailChamp\Model\Setting::updateLicense($request->license);
        ...
        return view('admin.settings.license', [
            'settings' => $settings,
            'current_license' => \MailChamp\Model\Setting::get('license'),
            ...
            foreach ($request->all() as $name => $value) {
                if ($name != '_token' && isset($settings[$name])) {
                    ...
                    if ($name == 'site_logo_small' || $name == 'site_logo_big') {
                        if ($request->hasFile($name) && $request->file($name)->isValid()) {
                            \MailChamp\Model\Setting::uploadSiteLogo($request->file($name), $name);
                        }
                    } else {
    ...
    public function advancedUpdate(Request $request, $name)
    {
        ...
        \MailChamp\Model\Setting::set($name, $request->value);
