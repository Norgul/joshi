...
class BlacklistController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (!$admin->can('read', new \MailChamp\Model\Blacklist())) {
    ...
    {
        if (!$request->user()->admin->can('read', new \MailChamp\Model\Blacklist())) {
            return $this->notAuthorized();
        }
    ...
    public function import(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            // authorize
            if (!$admin->can('import', new \MailChamp\Model\Blacklist())) {
                return $this->notAuthorized();
            }

            if ($request->hasFile('file')) {
                // Start system job
                $job = new \MailChamp\Jobs\ImportBlacklistJob($request->file('file')->path(), NULL, $admin);
                $this->dispatch($job);
            } else {
    ...
    public function cancel(Request $request)
    {
        ...
        // authorize
        if (!$admin->can('importCancel', new \MailChamp\Model\Blacklist())) {
            return $this->notAuthorized();
        }
