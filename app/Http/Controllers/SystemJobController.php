...
class SystemJobController extends Controller
{
    ...
    {
        $system_jobs = \MailChamp\Model\SystemJob::search($request)->paginate($request->per_page);
    ...
    {
        $job = \MailChamp\Model\ImportSubscribersSystemJob::find($request->id);

        // authorize
        // @todo: redundant find() here
        if (\Gate::denies('downloadImportLog', \MailChamp\Model\SystemJob::find($request->id))) {
    ...
    {
        $jobs = \MailChamp\Model\SystemJob::whereIn('id', explode(',', $request->uids));
    ...
    {
        $job = \MailChamp\Model\SystemJob::find($request->id);
