...
class SubscriberController extends Controller
{
    ...
    public function index(Request $request, $list_id)
    {
        ...
        $list = \MailChamp\Model\MailList::findByUid($list_id);
        ...
        if (isset($request->per_page)) {
            ...
            $per_page = \MailChamp\Model\Subscriber::$itemsPerPage;
        }

        $items = \MailChamp\Model\Subscriber::search($request, $user->customer)
    ...
    public function store(Request $request, $list_id)
    {
        try {
            ...
            $subscriber = \MailChamp\Model\Subscriber::where('mail_list_id', $list->id)->where('email', '=', $request->EMAIL)->first();
            ...
            if ($request->isMethod('post')) {
                ...
                if ($list->subscribe_confirmation) {
                    ...
                    try {
                        $subscriber->status = \MailChamp\Model\Subscriber::STATUS_UNCONFIRMED;
                        $subscriber->save();
                    ...
                    $subscriber->status = \MailChamp\Model\Subscriber::STATUS_SUBSCRIBED;
                    $subscriber->save();
                }
    ...
    public function show($list_id, $id)
    {
        ...
        if (strpos($id, '@') !== false) {
            $item = \MailChamp\Model\Subscriber::
                where('email', '=', $id)
                ->where('mail_list_id', '=', $list->id)
                ->first();
        } else {
    ...
    public function subscribe($list_id, $id)
    {
        ...
        $subscriber = \MailChamp\Model\Subscriber::findByUid($id);
    ...
    public function delete($list_id, $id)
    {
        ...
        if (!is_object($subscriber)) {
            $subscriber = \MailChamp\Model\Subscriber::where('mail_list_id', '=', $list->id)->where('email', '=', $id)->first();
        }
    ...
    public function showByEmail($email)
    {
        ...
        $subscribers = \MailChamp\Model\Subscriber::where('email', '=', $email)->get();
