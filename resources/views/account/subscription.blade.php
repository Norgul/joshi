    ...
    <div class="row">
        <div class="col-sm-12 col-md-8 col-lg-8">
            ...
            <div class="sub-section">
                    ...
                    @elseif (!$subscription->isPaid())
                                        ...
                                        <h5 class="mt-0 mb-0 text-semibold">
                                            <mc:flag>
                                                @if ($subscription->price)
                                                    {{ MailChamp\Library\Tool::format_price($subscription->price, $subscription->currency_format) }}
                                                @else
                                                    {{ trans('messages.free') }}
                                                @endif
                                            </mc:flag>
                        ...
                        @if (!$subscription->isPaid() && !$subscription->isActive())
                            @if (Auth::user()->customer->can('pay', $subscription))
                                ...
                                @foreach (\MailChamp\Model\PaymentMethod::getAllActive() as $payment_method)
                                    @if ($payment_method->type != \MailChamp\Model\PaymentMethod::TYPE_CASH)
                                        @if (
                ...
                @else
                    {!! trans('messages.current_plan_intro' . ($subscription->isTimeUnlimited() ? '_unlimited' : ''), [
                        'plan' => $subscription->plan_name,
                        'remain' => ($subscription->end_at ? \MailChamp\Library\Tool::dateTime($subscription->end_at)->diffForHumans(null, true) : 0),
                        'end_at' => \MailChamp\Library\Tool::formatDate($subscription->end_at)
                    ]) !!}
                    ...
                    <ul class="dotted-list topborder section">
                        ...
                        <li class="selfclear">
                            ...
                            <div class="lastUnit size1of2">
                                <mc:flag>{{ $subscription->start_at ? \MailChamp\Library\Tool::dateTime($subscription->start_at)->format(trans('messages.date_format')) : '' }}</mc:flag>
                            </div>
                        </li>
                        @if (!$subscription->isTimeUnlimited())
                            <li class="selfclear">
                                ...
                                <div class="lastUnit size1of2">
                                    <mc:flag>{{ $subscription->end_at ? \MailChamp\Library\Tool::dateTime($subscription->end_at)->format(trans('messages.date_format')) : '' }}</mc:flag>
                                </div>
                            </li>
                            <li>
                                <div class="unit size1of2">
                        ...
                        <li class="hide">
                            ...
                            <div class="lastUnit size1of2">
                                <mc:flag><span class="">{{ \MailChamp\Library\Tool::format_number(Auth::user()->customer->getSendingQuotaUsage()) }}/{{ (Auth::user()->customer->getSendingQuota() == -1) ? '∞' : \MailChamp\Library\Tool::format_number(Auth::user()->customer->getSendingQuota()) }}</span>
                        ...
                        <li class="hide">
                            ...
                            <div class="lastUnit size1of2">
                                <mc:flag><span class="">{{ \MailChamp\Library\Tool::format_number(Auth::user()->customer->listsCount()) }}/{{ \MailChamp\Library\Tool::format_number(Auth::user()->customer->maxLists()) }}</span>
                        ...
                        <li class="hide">
                            ...
                            <div class="lastUnit size1of2">
                                <mc:flag><span class="">{{ \MailChamp\Library\Tool::format_number(Auth::user()->customer->readCache('SubscriberCount', 0)) }}/{{ \MailChamp\Library\Tool::format_number(Auth::user()->customer->maxSubscribers()) }}</span>
                        ...
                        <li class="hide">
                            ...
                            <div class="lastUnit size1of2">
                                <mc:flag><span class="">{{ \MailChamp\Library\Tool::format_number(Auth::user()->customer->campaignsCount()) }}/{{ \MailChamp\Library\Tool::format_number(Auth::user()->customer->maxCampaigns()) }}</span>
                        ...
                        <li class="hide">
                            ...
                            <div class="lastUnit size1of2">
                                <mc:flag><span class="">{{ \MailChamp\Library\Tool::format_number(Auth::user()->customer->automationsCount()) }}/{{ \MailChamp\Library\Tool::format_number(Auth::user()->customer->maxAutomations()) }}</span>
                        ...
                        <li class="hide">
                            ...
                            <div class="lastUnit size1of2">
                                <mc:flag><span class="text-muted progress-xxs">{{ \MailChamp\Library\Tool::format_number(round(Auth::user()->customer->totalUploadSize(),2)) }}/{{ \MailChamp\Library\Tool::format_number(Auth::user()->customer->maxTotalUploadSize()) }} (MB)</span>
                            ...
                            <div class="lastUnit size1of2">
                                <mc:flag>
                                    @if (Auth::user()->customer->can("create", new MailChamp\Model\SendingServer()))
                                        <span class="">{{ \MailChamp\Library\Tool::format_number(Auth::user()->customer->sendingServersCount()) }}/{{ \MailChamp\Library\Tool::format_number(Auth::user()->customer->maxSendingServers()) }}</span>
                        ...
                        <li class="hide">
                            ...
                            <div class="lastUnit size1of2">
                                <mc:flag>
                                    @if (Auth::user()->customer->can("create", new MailChamp\Model\SendingDomain()))
                                        <span class="text-muted">{{ \MailChamp\Library\Tool::format_number(Auth::user()->customer->sendingDomainsCount()) }}/{{ \MailChamp\Library\Tool::format_number(Auth::user()->customer->maxSendingDomains()) }}</span>
                        ...
                        <li class="hide">
                            ...
                            <div class="lastUnit size1of2">
                                <mc:flag>
                                    @if (Auth::user()->customer->can("create", new MailChamp\Model\EmailVerificationServer()))
                                        <span class="">{{ \MailChamp\Library\Tool::format_number(Auth::user()->customer->emailVerificationServersCount()) }}/{{ \MailChamp\Library\Tool::format_number(Auth::user()->customer->maxEmailVerificationServers()) }}</span>
            ...
            @if (!$subscription->isTimeUnlimited())
                <div class="sub-section">
                    @if (!$nextSubscription)
                        @if (\Auth::user()->customer->can('create', new MailChamp\Model\Subscription()))
                                        ...
                                        <h5 class="mt-0 mb-0 text-semibold">
                                            <mc:flag>
                                                @if ($nextSubscription->price)
                                                    {{ MailChamp\Library\Tool::format_price($nextSubscription->price, $nextSubscription->currency_format) }}
                                                @else
                                                    {{ trans('messages.free') }}
                                                @endif
                                            </mc:flag>
                    ...
                    @else
                        ...
                        <p>
                            {!! trans('messages.next_plan_active_intro', [
                                'plan' => $nextSubscription->plan_name,
                                'remain' => \MailChamp\Library\Tool::dateTime($nextSubscription->end_at)->diffForHumans(null, true),
                                'start_at' => \MailChamp\Library\Tool::formatDate($nextSubscription->start_at)
                            ]) !!}
                        </p>
