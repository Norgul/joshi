...
<html lang="en">
<head>
	<title>@yield('title') - {{ \MailChamp\Model\Setting::get("site_name") }}</title>
	...
	<div class="navbar navbar-{{ Auth::user()->customer->getColorScheme() == "white" ? "default" : "inverse" }} navbar-fixed-top">
		<div class="navbar-header">
			<a class="navbar-brand" href="{{ action('HomeController@index') }}">
				@if (\MailChamp\Model\Setting::get('site_logo_small'))
                    <img src="{{ action('SettingController@file', \MailChamp\Model\Setting::get('site_logo_small')) }}" alt="">
                @else
		...
		<div class="navbar-collapse collapse" id="navbar-mobile">
			<ul class="nav navbar-nav">
				...
				@if (
					Auth::user()->customer->can("read", new MailChamp\Model\SendingServer()) ||
					Auth::user()->customer->can("read", new MailChamp\Model\SendingDomain()) ||
                    Auth::user()->customer->can("read", new MailChamp\Model\EmailVerificationServer()) ||
					Auth::user()->customer->can("read", new MailChamp\Model\Blacklist()) ||
					true
				)
					...
					>
						...
						<ul class="dropdown-menu">
							@if (Auth::user()->customer->can("read", new MailChamp\Model\SendingServer()))
							...
							@endif
							@if (Auth::user()->customer->can("read", new MailChamp\Model\SendingDomain()))
                            ...
                            @if (Auth::user()->customer->can("read", new MailChamp\Model\EmailVerificationServer()))
							...
							@endif
							@if (Auth::user()->customer->can("read", new MailChamp\Model\Blacklist()))
			...
			<ul class="nav navbar-nav navbar-right">
				<!--<li class="dropdown language-switch">
					<a class="dropdown-toggle" data-toggle="dropdown">
						{{ MailChamp\Model\Language::getByCode(Config::get('app.locale'))->name }}
						<span class="caret"></span>
					</a>

					<ul class="dropdown-menu">
						@foreach(MailChamp\Model\Language::getAll() as $language)
							<li class="{{ MailChamp\Model\Language::getByCode(Config::get('app.locale'))->code == $language->code ? "active" : "" }}">
								<a>{{ $language->name }}</a>
							</li>
						@endforeach
					</ul>
				...
				<li class="dropdown dropdown-user">
					...
					<ul class="dropdown-menu dropdown-menu-right">
						...
						@if (Auth::user()->customer->can("read", new MailChamp\Model\Subscription()))
