                        ...
                        >
                            ...
                            @foreach ($fields as $key => $item)
                                <tr class="draggable" rel="{{ $item->uid }}">
                                    ...
                                    <td class="text-nowrap">
										@include('helpers.form_control', ['type' => MailChamp\Model\Field::getControlNameByType($item->type), 'name' => 'fields[' . $item->uid . '][default_value]', 'label' => '', 'value' => $item->default_value, 'help_class' => 'field'])
									</td>
									<td>
										@if ($item->tag != 'EMAIL')
											@if (is_object(MailChamp\Model\Field::findByUid($item->uid)))
