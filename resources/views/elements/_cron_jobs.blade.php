...
@if(!$valid || isset($show_all))
    @include('helpers.form_control', [
        ...
        'value' => $queue_driver,
        'options' => \MailChamp\Library\Tool::availableSystemBackgroundMethodSelectOptions(),
...
<div class="database-config-box">
    @if(!$valid || isset($show_all))
        @include('helpers.form_control', [
            ...
            'value' => $php_bin_path,
            'options' => \MailChamp\Library\Tool::phpPathsSelectOptions($php_paths),
        ...
        @if ($errors->has('php_bin_path_invalid'))
            <div class="alert alert-danger">
                <?php $check = \MailChamp\Library\Tool::checkPHPBinPath($php_bin_path_value); ?>
                @if ($check == '')
    ...
    @if($valid || (isset($show_all) && !$errors->has('php_bin_path_invalid')))
        <div id="result_box">
            ...
            <div class="form-group mb-10">
                <pre class="remote-job-url" style="font-size: 16px;background:#f5f5f5">{{ action('Controller@remoteJob', ['remote_job_token' => \MailChamp\Model\Setting::get('remote_job_token')]) }}</pre>
            </div>
